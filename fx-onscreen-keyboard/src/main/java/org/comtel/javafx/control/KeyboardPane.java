...
public class KeyboardPane extends Region implements StandardKeyCode, EventHandler<KeyButtonEvent>
{
  ...
  //TODO currently the keyboard does not compute its preferred dimensions which
  //makes it impossible to add it to a layouted panel and look good. (The
  //layout manager would need the sizes to reserve enough space for the keyboard)
  public static final int KEYBOARD_PREFERRED_HEIGHT = 280;

  private static final String DEFAULT_CSS = "/css/KeyboardButtonStyle.css";//NOI18N
  private static final String DEFAULT_FONT_URL = "/font/FontKeyboardFX.ttf";//NOI18N
  ...
  public KeyboardPane()
  {
    ...
    setPrefHeight(KEYBOARD_PREFERRED_HEIGHT);
  ...
  public void load() throws MalformedURLException, IOException, URISyntaxException
  {
    ...
    if (layerPathProperty.get() == null) {
      String fontUrl = KeyboardPane.class.getResource(DEFAULT_FONT_URL).toExternalForm();
      Font font = Font.loadFont(fontUrl, -1);
      if (font == null) {
        throw new IllegalStateException("Font " + DEFAULT_FONT_URL + " not found!");
      }
    }
  ...
  public void setLayoutLocale(Locale local) throws MalformedURLException, IOException, URISyntaxException
  {
    ...
    if (layerPathProperty.get() == null) {
      StringBuilder xmlPath = new StringBuilder("/xml/");
      xmlPath.append(layerProperty.get().toString().toLowerCase(Locale.ENGLISH));
      String languageTag = local.toLanguageTag();
      xmlPath.append("/");
      if (!languageTag.equals("en")) {
        xmlPath.append(languageTag.replace('-', '_'));//we must replace the - with a _. Java does not allow - in package names and the resources are stored in a package named equal to the language tag
        xmlPath.append("/");
      }
      logger.info("use embedded layouts path: {}", xmlPath.toString());
    ...
    else if (localMap.containsKey(
            new Locale(local.getLanguage()))) {
  ...
  private Region createKeyboardPane(Keyboard layout)
  {
    ...
    for (Keyboard.Row row : layout.getRow()) {
      ...
      for (Keyboard.Row.Key key : row.getKey()) {
        ...
        MultiKeyButton button = new MultiKeyButton(this, scaleProperty, getStylesheets());
        ...
        if (key.getKeyIconStyle() != null && key.getKeyIconStyle().startsWith(".")) {
          ...
          InputStream is = KeyboardPane.class
